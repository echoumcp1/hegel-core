...
def test_cli_integration():
    """Test the CLI server can be started and used."""
    import subprocess
    import tempfile
    import os
    import time
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        socket_path = os.path.join(tmpdir, "hegel.sock")

        # Start the server in a subprocess
        server_proc = subprocess.Popen(
            ["python", "-m", "hegel", "--socket", socket_path, "--verbosity", "quiet"],
        ...
        try:
            ...
            client_connection = Connection(sock, name="Client")
            client = Client(client_connection)
            ...
            result = client.run_test("cli_test", my_test, test_cases=10)
            assert result.passed
